...
from src.monitoring import get_metrics_registry, setup_logging
...
async def get_version():
    ...
    return {
        "version": "1.0.0",
        "algorithm_version": (
            clustering_algorithm.get_version() if clustering_algorithm else "unknown"
        ),
...
@app.post(
    "/api/v1/cluster/process", response_model=ClusterResponse, tags=["Clustering"]
)
...
async def process_clustering(
    ...
    logger.info(
        f"Processing clustering request {job_id} with {len(request.data)} points"
    )
    ...
    try:
        ...
        if request.algorithm == "ncs":
            result = await clustering_algorithm.cluster_ncs(
                data_array, **request.params.dict(exclude_none=True)
            )
        ...
        job_store[job_id].update(
            {"status": "completed", "progress": 1.0, "updated_at": datetime.utcnow()}
        )
        ...
        response = ClusterResponse(
            ...
            status="completed",
            clusters=(
                result["clusters"].tolist() if result["clusters"] is not None else None
            ),
            centroids=(
                result["centroids"].tolist()
        ...
        await connection_manager.broadcast(
            {"type": "job_completed", "job_id": job_id, "result": response.dict()}
        )
        ...
        job_store[job_id].update(
            {"status": "failed", "updated_at": datetime.utcnow(), "error": str(e)}
        )
...
async def get_metrics():
    ...
    return {
        "active_jobs": len(
            [j for j in job_store.values() if j["status"] == "processing"]
        ),
        "total_jobs": len(job_store),
        "completed_jobs": len(
            [j for j in job_store.values() if j["status"] == "completed"]
        ),
