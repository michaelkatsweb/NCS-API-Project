    ...
    def get_statistics(self, db: Session, session_id: uuid.UUID) -> Dict[str, Any]:
        ...
        try:
            ...
            return {
                ...
                "outlier_percentage": (
                    (stats.outlier_count / stats.total_points * 100)
                ...
                "avg_confidence": (
                    float(stats.avg_confidence) if stats.avg_confidence else 0
                ),
                "avg_processing_time_ms": (
                    float(stats.avg_processing_time) if stats.avg_processing_time else 0
                ),
                "max_processing_time_ms": (
                    float(stats.max_processing_time) if stats.max_processing_time else 0
                ),
                "min_processing_time_ms": (
                    float(stats.min_processing_time) if stats.min_processing_time else 0
                ),
            }
        except SQLAlchemyError as e:
        ...
        try:
            ...
            return {
                ...
                "avg_session_duration_seconds": (
                    float(stats.avg_duration) if stats.avg_duration else 0
                ),
                "total_data_processed_mb": (
                    float(stats.total_data_mb) if stats.total_data_mb else 0
                ),
                "avg_success_rate": (
                    float(stats.avg_success_rate) if stats.avg_success_rate else 0
                ),
            }
        except SQLAlchemyError as e:
