...
class ClusteringParams(BaseModel):
    ...
    # Common parameters
    n_clusters: Optional[int] = Field(None, ge=1, le=100, description="Number of clusters")
    quality_threshold: float = Field(0.85, ge=0.0, le=1.0, description="Minimum quality threshold")
    max_iterations: int = Field(100, ge=1, le=1000, description="Maximum iterations")
    random_seed: Optional[int] = Field(None, description="Random seed for reproducibility")
    
    # NCS-specific parameters
    auto_cluster_detection: bool = Field(True, description="Enable automatic cluster detection")
    convergence_tolerance: float = Field(1e-6, gt=0, description="Convergence tolerance")
    
    # DBSCAN-specific parameters
    eps: float = Field(0.5, gt=0, description="Maximum distance between points in neighborhood")
    min_samples: int = Field(5, ge=1, description="Minimum samples in neighborhood")
    
    # K-Means specific parameters
    init_method: str = Field("k-means++", regex="^(k-means\+\+|random)$", description="Initialization method")
...
class ClusterRequest(BaseModel):
    ...
    algorithm: ClusteringAlgorithm = Field(
        ClusteringAlgorithm.NCS,
        description="Clustering algorithm to use"
    )
    params: ClusteringParams = Field(
        default_factory=ClusteringParams,
        ...
        if len(v) > 1:
            ...
            for i, row in enumerate(v[1:], 1):
                if len(row) != first_dim:
                    raise ValueError(f"Row {i} has {len(row)} dimensions, expected {first_dim}")
...
class ClusterResponse(BaseModel):
    ...
    clusters: Optional[List[int]] = Field(None, description="Cluster assignments for each point")
    centroids: Optional[List[List[float]]] = Field(None, description="Cluster centroids")
    quality_score: Optional[float] = Field(None, ge=0.0, le=1.0, description="Clustering quality score")
...
class JobStatusResponse(BaseModel):
    ...
    result: Optional[ClusterResponse] = Field(None, description="Job result if completed")
    error: Optional[str] = Field(None, description="Error message if failed")
...
class BatchClusterResponse(BaseModel):
    ...
    jobs: List[JobStatusResponse] = Field(..., description="Status of each job in batch")
...
class AlgorithmInfo(BaseModel):
    ...
    supports_auto_clusters: bool = Field(..., description="Supports automatic cluster detection")
...
class MetricsResponse(BaseModel):
    ...
    points_processed_per_second: Optional[float] = Field(None, description="Processing throughput")
