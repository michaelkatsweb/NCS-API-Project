...
class NCSClusteringAlgorithm:
    ...
    async def cluster_ncs(
        ...
        **kwargs
        ...
        logger.info(f"NCS clustering completed in {processing_time:.2f}ms with quality {quality:.3f}")
        ...
        **kwargs
        ...
        if np.sum(non_noise_mask) > 1 and n_clusters > 1:
            quality = self._calculate_silhouette_score(
                data_normalized[non_noise_mask], 
        ...
        for size in data_sizes:
            ...
            for algorithm in algorithms:
                try:
                    ...
                    logger.error(f"Benchmark error for {algorithm} with size {size}: {e}")
                    results[algorithm].append({
                        "data_size": size,
                        "error": str(e)
                    })
