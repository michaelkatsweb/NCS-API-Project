...
class ConnectionManager:
    ...
    async def broadcast(
        ...
        for connection_id, connection in self.connections.items():
            if connection_id not in exclude:
                send_tasks.append(
                    self._safe_send(connection, message_with_meta, connection_id)
                )
        ...
        logger.info(
            f"Broadcast sent to {len(send_tasks) - len(failed_connections)} connections"
        )
    ...
    async def broadcast_to_topic(self, topic: str, message: Dict[str, Any]):
        ...
        for connection_id in self.topics[topic]:
            if connection_id in self.connections:
                connection = self.connections[connection_id]
                send_tasks.append(
                    self._safe_send(connection, message_with_topic, connection_id)
                )
            else:
                failed_connections.append(connection_id)
    ...
    async def _ping_all_connections(self):
        ...
        for connection_id, connection in self.connections.items():
            try:
                ...
                if current_time - connection.last_ping > self.ping_interval * 2:
                    logger.warning(
                        f"Connection {connection_id} appears stale, removing"
                    )
                    failed_connections.append(connection_id)
                    continue
        ...
        if self.connections:
            logger.debug(
                f"Pinged {len(self.connections)} connections, removed {len(failed_connections)}"
            )
    ...
    def get_stats(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "topic_stats": {
                topic: len(connections) for topic, connections in self.topics.items()
            },
        }
